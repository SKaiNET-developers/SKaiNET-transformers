...
class Gemma4BlockTraceProbe {
    ...
    private class Capture : ForwardHooks {
        ...
        override fun onForwardEnd(module: ModuleNode, input: Any, output: Any) {
            ...
            if (t.shape.volume > 20000) return
    ...
    fun per_block_trace() {
        ...
        println("TRACE ref attn_norm-0 [+8.2090, -10.8485, -16.9405]  Qn [+0.0960, +0.6962, +0.3737]  Kn [+0.4673, -0.2566, -0.0489]")
        println("TRACE ref Vn [-0.7105, -1.4492, -0.9298]  Qrope [+0.9449, +0.2218, +0.3428]  Krope [+0.4309, -0.2379, -0.1139]  attnOut [-0.7368, -0.5313, -0.8144]  l_out-0 [+0.0299, -0.2452, -0.0961]")
    }
}
