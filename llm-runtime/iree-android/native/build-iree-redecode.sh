#!/usr/bin/env bash
# Cross-build libskainet_iree_redecode.so for an Android ABI using the consolidated
...
docker run --rm \
  ...
  build-so "$ABI" --name skainet_iree_redecode --src /src/iree_redecode_jni.c $VULKAN \
