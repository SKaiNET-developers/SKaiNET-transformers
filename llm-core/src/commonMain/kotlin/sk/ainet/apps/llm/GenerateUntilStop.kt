 ...
 *
 * Promoted from `llm-agent` (issue #49 Phase 1) so that any runner with an
 * [InferenceRuntime] gets stop-token-aware generation without depending on
 * the agent layer. The `sk.ainet.apps.kllama.agent.generateUntilStop`
 * extension in `llm-agent` now delegates here.
 *
 ...
 *
 * Delegates to the shared [sampleFromLogits] after extracting the float array.
 *
