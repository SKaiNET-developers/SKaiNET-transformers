...
:description: Implement and register a new compute backend (Metal GPU, MLX, CUDA, Vulkan) for the SKaiNET inference engine.

This guide explains how to implement and register a new compute backend for the SKaiNET inference engine.
...
| JVM
| `java.util.ServiceLoader` — auto-discovers from JAR

| Native (macOS, Linux, iOS)
| `registerPlatformBackends()` at startup

| Android, JS, Wasm
| `BackendRegistry.register()` called by host app
|===
...
sourceSets {
    val jvmMain by getting {
        dependencies {
            implementation("sk.ainet.core:skainet-backend-metal:0.21.0")
        }
    }
...
If the backend wraps a native C/C{plus}{plus} library (Metal, MLX, Vulkan),
...
* [ ] Native: linkerOpts if wrapping C/C{plus}{plus} bridge
