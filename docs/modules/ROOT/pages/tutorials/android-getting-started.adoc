...
| Confirm you're using `AndroidRandomAccessSource` + `loadToMapStreaming`, not a sequential `Source` + `loadToMap` — the sequential path materializes the whole file (and always dequantizes). Also confirm you didn't pass `DECODER_DEQUANTIZE_ALL` as the weight form (4–8x the memory of the packed default).
