...
public fun main() {
    ...
    val hostGather = (System.getenv("QWEN_HOST_GATHER") ?: "0").lowercase() in setOf("1", "true")

    when (graph) {
        "prefill_at" -> QwenExportHarness.exportPrefillAt(gguf, outDir, seq, bf16, hostGather)
        "prefill_with_past" -> QwenExportHarness.exportPrefillWithPast(gguf, outDir, chunk, bf16, hostGather)
        "with_past" -> QwenExportHarness.exportWithPast(gguf, outDir, bf16, hostGather)
        "all" -> QwenExportHarness.exportAll(gguf, outDir, seq, chunk, bf16, hostGather)
        else -> error("QWEN_GRAPH must be one of prefill_at|prefill_with_past|with_past|all, got '$graph'")
    }
    println("[QwenExportCli] wrote $graph to $outDir (dtype=${if (bf16) "bf16" else "fp32"}, hostGather=$hostGather)")
}
